...
-- (Tabel Surat Pengantar / Antrian Pelayanan Kosong - Tanpa Data Dummy)
...
-- --------------------------------------------------------
-- RPC FUNCTION: SINKRONISASI KREDENSIAL ADMIN DENGAN AMAN
-- Menggunakan SECURITY DEFINER agar tidak perlu membuka RLS INSERT/UPDATE ke publik.
-- --------------------------------------------------------
CREATE OR REPLACE FUNCTION public.sync_admin_credentials(
  p_username TEXT,
  p_password_hash TEXT
) RETURNS BOOLEAN SECURITY DEFINER AS $$
BEGIN
  INSERT INTO public.admin_users (username, password, nama_admin)
  VALUES (LOWER(TRIM(p_username)), p_password_hash, 'Pengurus RW 09')
...
GRANT EXECUTE ON FUNCTION public.sync_admin_credentials(TEXT, TEXT) TO anon, authenticated;
