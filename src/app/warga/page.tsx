...
export default function WargaPage() {
  ...
  // Form State (Input & simpan ke DB menggunakan Tahun Lahir)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Save to Supabase (Menyimpan nama, tahun_lahir, rt ke DB)
    if (isSupabaseConfigured && supabase) {
      try {
        if (editingWarga) {
          await supabase
    ...
    if (editingWarga) {
      setWargaList((prev) =>
        prev.map((w) => (w.id === editingWarga.id ? { id: w.id, nama: formData.nama, tahunLahir: formData.tahunLahir, rt: formData.rt } : w))
      );
    } else {
  ...
  // Filtered List & Age Calculation (Tahun Sekarang - Tahun Lahir)
      ...
      <main className="ml-[280px] w-[calc(100%-280px)] min-h-screen flex flex-col">
        ...
        <div className="flex-1 p-8 space-y-8 overflow-y-auto">
          ...
          <div className="bg-white rounded-xl border border-[#e2e2e2] overflow-hidden shadow-sm">
            ...
            {/* Table (Hanya Menampilkan Usia: Tahun Sekarang - Tahun Lahir) */}
